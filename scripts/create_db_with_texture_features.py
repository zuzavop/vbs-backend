...
def extend_with_texture_features(db_dir, model):
    ...
    for file in pkl_files:
        ...
        try:
            ...
            # Create a separate file for texture data based on the original database file
            localdata_storage = f'{internal_storage[:-4]}_{type_local_data}_texture.pkl'

            # Check conditions for local data existence or validity in the data
            if (
                not os.path.exists(localdata_storage)
                or data.TEXTURE_DATA is None
                or data.TEXTURE_DATA == ''
                or not hasattr(data, 'TEXTURE_DATA')
                or type_local_data not in data.TEXTURE_DATA
            ):
                features = []

                print(f'Starting to add texture features to the database')
                ...
                for id in tqdm(ids):
                    ...
                    local_data_file = os.path.join(os.path.join(db_dir, video_id),video_id + "-clip-vit-so400m.hdf5")
                ...
                if not isinstance(data.TEXTURE_DATA, dict):
                    data.TEXTURE_DATA = {}
                data.TEXTURE_DATA[type_local_data] = localdata_storage
                ...
                print(f'Already contains texture features {internal_storage}')
...
if __name__ == '__main__':
    ...
    extend_with_texture_features(db_dir, model)
